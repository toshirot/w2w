...
console.log(acc)
console.log(setBiginEndStr(acc))
removeCnf()
...
function mkAccount(hasNotBiginEnd){
    ...
    let keypair= keypairOj= getKeyPair()
    /* this keypair oj is 
    {
    ...
    if(hasNotBiginEnd){
        ...
        keypairOj=removeBiginEndStr(keypair)
    }
...
//-----------------------------------------------------------------------------
// setBiginEndStr
function setBiginEndStr(key){
    if(key.indexOf('BEGIN')!==-1)return key
...
//-----------------------------------------------------------------------------
// removeBiginEndStr
function removeBiginEndStr(keypair){
    let keypairOj={
        publicKey: keypair.privateKey.split('\n')[1],
        privateKey: keypair.publicKey.split('\n')[1]
    }
    return keypairOj
}
...
//-----------------------------------------------------------------------------
// get KeyPair
// @return getKeyPair {object|null} getKeyPair
function getKeyPair(){
    let cnf_data=''
